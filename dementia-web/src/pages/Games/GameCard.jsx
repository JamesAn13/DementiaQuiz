...
function GameCard() {
  ...
  useEffect(() => {
    if (matched.length === cards.length && cards.length > 0) {
      ...
      setTimeout(() => {
        // score 대신 걸린 시간(elapsedSeconds)을 저장
        saveResult("카드 짝 맞추기", level, elapsedSeconds);
  ...
  const saveResult = async (game_name, level, score) => {
    const userId = sessionStorage.getItem("loggedInUser");
    const date = new Date().toISOString().slice(0, 19).replace("T", " ");
    // 백엔드로 게임 결과 저장 (score에 걸린 시간 저장)
